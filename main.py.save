...
def store_alert(data):
    ...
    c.execute("""
        ...
        (timestamp, alert_type, direction, price, exit_pts,
         ...
         tl_spread, tl_state, rsi, vix_rsi, compare_rsi, adx,
         verdict, claude_analysis, claude_confidence, raw_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
    ...
    for r in rows:
        if r[2] > 0:
            if streak_dir == "" or streak_dir == "W":
                streak += 1; streak_dir = "W"
            else: break
        elif r[2] < 0:
            if streak_dir == "" or streak_dir == "L":
                streak += 1; streak_dir = "L"
            else: break
...
def parse_alert(raw_json):
    ...
    data = {
        ...
        "tl_spread": 0, "tl_state": "",
        "rsi": 0, "vix_rsi": 0, "compare_rsi": 0, "adx": 0,
        "verdict": "",
    ...
    try:
        ...
        if "GO LONG" in content or "RE-LONG" in content:
            data["alert_type"] = "ENTRY"; data["direction"] = "LONG"
        elif "GO SHORT" in content or "RE-SHORT" in content:
            data["alert_type"] = "ENTRY"; data["direction"] = "SHORT"
        ...
        if today_match: data["daily_pnl"] = float(today_match.group(1))

        week_match = re.search(r'Week:\s*([+-]?\d+\.?\d*)', content)
        if week_match: data["weekly_pnl"] = float(week_match.group(1))

        month_match = re.search(r'Month:\s*([+-]?\d+\.?\d*)', content)
        if month_match: data["monthly_pnl"] = float(month_match.group(1))

        total_match = re.search(r'Total:\s*([+-]?\d+\.?\d*)pts', content)
        if total_match: data["total_pnl"] = float(total_match.group(1))
        ...
        if rsi_match: data["rsi"] = float(rsi_match.group(1))

        vix_match = re.search(r'VIX:\s*(\d+)', content)
        if vix_match: data["vix_rsi"] = float(vix_match.group(1))

        cl_match = re.search(r'CL:\s*(\d+)', content)
        if cl_match: data["compare_rsi"] = float(cl_match.group(1))

        adx_match = re.search(r'ADX:\s*(\d+)', content)
        if adx_match: data["adx"] = float(adx_match.group(1))

        for v in ["\U0001f4aa", "\u26a1", "\U0001f4c8", "\U0001f6a8", "\u26a0\ufe0f", "\U0001f504", "\U0001f4ca"]:
            if v in content:
...
# DISCORD FORWARDING
# ═══════════════════════════════════════════════════
def forward_to_discord(original_json, claude_analysis="", claude_confidence=""):
    ...
    if isinstance(original_json, dict) and "content" in original_json:
        ...
        if claude_analysis:
            conf_emoji = (
                "\U0001f7e2" if claude_confidence == "HIGH" else
                "\U0001f7e1" if claude_confidence == "MEDIUM" else
                "\U0001f534"
            )
...
ENDOFFILE""
OneMP11 Alert Server — Intercepts TradingView alerts, enriches with Claude analysis, forwards to Discord
Deploy on Railway: https://railway.app
"""
import os
import json
...
DISCORD_WEBHOOK_URL = os.environ.get("DISCORD_WEBHOOK_URL", "")
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")
WEBHOOK_SECRET = os.environ.get("WEBHOOK_SECRET", "onemp11")  # Optional: verify TV webhooks
ENABLE_CLAUDE = os.environ.get("ENABLE_CLAUDE", "true").lower() == "true"
DB_PATH = os.environ.get("DB_PATH", "alerts.db")
...
def get_recent_alerts(n=10):
    ...
    rows = c.fetchall()
    conn.close()
    columns = ["id", "timestamp", "alert_type", "direction", "price", "exit_pts",
               "daily_pnl", "weekly_pnl", "monthly_pnl", "total_pnl",
               "tl_spread", "tl_state", "rsi", "vix_rsi", "compare_rsi", "adx",
               "verdict", "claude_analysis", "claude_confidence", "raw_json"]
    return [dict(zip(columns, row)) for row in rows]
...
def get_stats(days=7):
    ...
    c.execute("""
        SELECT alert_type, direction, exit_pts, claude_confidence
        FROM alerts
        WHERE timestamp > ? AND exit_pts != 0
        ORDER BY id DESC
    """, (cutoff,))
    rows = c.fetchall()
    conn.close()
...
def parse_alert(raw_json):
    ...
    try:
        ...
        # Extract verdict (milestone alerts)
        for v in ["💪", "⚡", "📈", "🚨", "⚠️", "🔄", "📊"]:
            if v in content:
                verdict_match = re.search(f'{re.escape(v)}\\s*(.+?)\\\\n', content)
                if not verdict_match:
...
def analyze_with_claude(alert_data, recent_alerts):
    """Send alert to Claude for analysis"""
    if not ANTHROPIC_API_KEY or not ENABLE_CLAUDE:
        return "", ""
...
"""
    ...
    prompt = f"""You are a professional ES futures trading analyst. Analyze this TradingView alert and provide:
1. A brief assessment (2-3 sentences max) of whether this alert supports the current trend
2. A confidence level: HIGH, MEDIUM, or LOW
...
Rules:
- For ENTRY alerts: assess if conditions support the trade direction
- For MILESTONE_UP alerts: assess hold vs take profit
- For MILESTONE_DOWN alerts: assess hold vs cut
- For SESSION_CLOSE/REVERSAL: assess the result and market state
- For SESSION_OPEN/MARKET_CHECK: assess conditions for the upcoming session
- Keep it brief and actionable — this goes to a Discord mobile notification
- Respond in EXACTLY this format:
ANALYSIS: [your 2-3 sentence analysis]
CONFIDENCE: [HIGH/MEDIUM/LOW]"""
...
def forward_to_discord(original_json, claude_analysis="", claude_confidence=""):
    ...
    if isinstance(original_json, dict) and "content" in original_json:
        ...
        if claude_analysis:
            conf_emoji = "🟢" if claude_confidence == "HIGH" else "🟡" if claude_confidence == "MEDIUM" else "🔴"
            content += f"\n━━━━━━━━━━━━━━━━━━━\n🤖 **CLAUDE** {conf_emoji} {claude_confidence}\n{claude_analysis}"
...
@app.route("/webhook", methods=["POST"])
def webhook():
    """Receive TradingView webhook, analyze with Claude, forward to Discord"""
    try:
        raw = request.get_json(force=True)
    except Exception:
        try:
    ...
    # Analyze with Claude (skip for low-value alerts)
    claude_analysis = ""
    claude_confidence = ""
    skip_types = ["NO_ENTRY", "TREND_OVER", "REGIME_SHIFT"]
    ...
    # Forward to Discord with analysis
    forward_to_discord(raw, claude_analysis, claude_confidence)
...
- Total P&L: {stats['total_pts']}pts
- Avg per trade: {stats['avg_pts']}pts
...
Keep it concise — 3-4 sentences total."""
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                summary = result["content"][0]["text"]
                return jsonify({"stats": stats, "summary": summary})
        except Exception as e:
            return jsonify({"stats": stats, "summary": f"Error generating summary: {e}"})

    return jsonify({"stats": stats, "summary": "Claude analysis disabled."})
